...
/**
 * Process incoming Twilio SMS webhook data
 */
export const processIncomingMessage = internalMutation({
  args: {
    webhookData: v.any(), // Twilio webhook payload
  },
  returns: v.object({
  ...
  handler: async (ctx, args) => {
    ...
    // Extract fields from Twilio webhook format
    const messageId = webhookData.MessageSid;
    const senderPhone = webhookData.From; // Phone number that sent the message
    const recipientPhone = webhookData.To; // Our Twilio number
    const messageText = webhookData.Body;
    const messageType = webhookData.MessageType || "text";
    const numMedia = Number.parseInt(webhookData.NumMedia || "0", 10);
    ...
    if (numMedia > 0) {
      console.log(
        `Ignoring message with media attachments (NumMedia: ${numMedia})`
      );

      // Send unsupported message type response
      await ctx.scheduler.runAfter(0, internal.sms.sendMessage, {
    ...
    console.log(
      `Processing ${messageType} SMS from ${senderPhone}: ${messageText}`
    );
    console.log(`Message ID: ${messageId}, To: ${recipientPhone}`);
    ...
    await ctx.scheduler.runAfter(0, internal.sms.generateResponse, {
...
export const generateResponse = internalAction({
  ...
  returns: v.null(),
  handler: async (ctx, args) => {
    const { userId, phoneNumber, messageText, messageId } = args;
    ...
    try {
      ...
      if (existingThreads.page.length > 0) {
        ...
        threadId = await createThread(ctx, components.agent, {
          userId: actualUserId,
          title: `SMS chat with ${phoneNumber}`,
          summary: `Fitness coaching conversation with ${phoneNumber} via SMS`,
        });
      ...
      await ctx.runAction(internal.sms.sendSplitMessages, {
        phoneNumber,
...
export const sendSplitMessages = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      console.log(
        `Sending ${messages.length} split SMS messages to ${phoneNumber}`
      );

      // Send each message with realistic delays for SMS
      for (let i = 0; i < messages.length; i++) {
        const message = messages[i];

        // Calculate typing delay based on message length (simulate realistic typing speed)
        // SMS is typically slower, so add a bit more delay
        // Average typing speed: ~40 WPM = ~200 characters per minute = ~3.3 chars per second
        const typingDelayMs = Math.max(
          1000, // Minimum delay of 1 second for SMS
          message.length * 60 + Math.random() * 500 // ~60ms per character + random variance
        );
        ...
        console.log(
          `Sending SMS ${i + 1}/${messages.length}: "${message}" (after ${i > 0 ? typingDelayMs : 0}ms delay)`
        );
      ...
      console.log(
        `Successfully sent all ${messages.length} SMS messages to ${phoneNumber}`
      );
    } catch (error) {
      console.error("Error sending split SMS messages:", error);
 ...
 */
export const handleStatusCallback = internalAction({
  args: {
    webhookData: v.any(),
  },
  returns: v.null(),
  handler: (_ctx, args) => {
    ...
    const messageId = webhookData.MessageSid;
    const status = webhookData.MessageStatus; // queued, sending, sent, delivered, undelivered, failed
    const errorCode = webhookData.ErrorCode;
    const errorMessage = webhookData.ErrorMessage;
    const to = webhookData.To;

    console.log(
      `Message ${messageId} status update: ${status} for recipient ${to}`
    );

    if (errorCode) {
      console.error(
        `Message ${messageId} error: Code ${errorCode} - ${errorMessage}`
      );
    }
